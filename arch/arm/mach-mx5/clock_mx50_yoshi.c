...
static struct clk apll_clk = {
	...
	.flags = RATE_PROPAGATES | AHB_HIGH_SET_POINT | CPU_FREQ_TRIG_UPDATE,
};
...
static void cko2_recalc(struct clk *clk)
{
	unsigned long rate;
	u32 reg;
...
static int cko2_set_rate(struct clk *clk, unsigned long rate)
{
	...
	printk("cko2_set_rate, new divider=%d\n",div);
...
int __init mx50_clocks_init(unsigned long ckil, unsigned long osc, unsigned long ckih1)
{
	...
	clk_set_rate(&ahb_clk, 133333333);
	propagate_rate(&ahb_clk);
	...
	propagate_rate(&esdhc1_clk[0]);
	propagate_rate(&esdhc3_clk[0]);

	clk_set_rate(&cko2_clk, 4000000);
	clk_set_parent(&cko2_clk, &lp_apm_clk);
...
void mx50_audio_clock_enable(int enable)
{
	if (!enable)
		cko2_disable(&cko2_clk);
	else
		cko2_enable(&cko2_clk);
}

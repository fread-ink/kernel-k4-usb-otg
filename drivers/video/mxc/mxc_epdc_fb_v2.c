 ...
 *
 * Version of the driver that eliminates the eINK HAL top layer
...
static int mxc_epdc_update_mode = UPDATE_MODE_FULL;
...
static int epdc_powerup_status = 0; /* Lab126 */
...
static void epdc_powerdown(struct mxc_epdc_fb_data *fb_data)
{
	...
	/* Lab126: Check to see if all updates have completed */
	if (!is_free_list_full(fb_data) &&
		(fb_data->cur_update != NULL) &&
		epdc_any_luts_active() && !epdc_powerup_status) {
		mutex_unlock(&fb_data->power_mutex);
		return;
	}
				   ...
				   struct fb_info *info)
{
	...
	if (list_empty(&fb_data->upd_buf_free_list->list)) {
		dev_err(fb_data->dev,
			"E send_update:def:edpc_intermedia_buffer=empty:\n");
		spin_unlock_irqrestore(&fb_data->queue_lock, flags);
		return -ENOMEM;
	}
			     ...
			     unsigned long arg)
{
	...
	case MXCFB_SET_UPDATE_FULL:
		{
			mxc_epdc_update_mode = UPDATE_MODE_FULL;
			break;
		}
	case MXCFB_SET_UPDATE_PARTIAL:
		{
			mxc_epdc_update_mode = UPDATE_MODE_PARTIAL;
			break;
		}
	default:
		break;
				     ...
				     u16 y1, u16 y2)
{
	...
	/* Lab126 */
	__u32 update_mode = fb_data->auto_mode == AUTO_UPDATE_MODE_AUTOMATIC_MODE_FULL ? 
							UPDATE_MODE_FULL :
							UPDATE_MODE_PARTIAL;	
	...
	update.update_mode = update_mode; /* Lab126 */
...
{
	struct mxc_epdc_fb_data *fb_data = (struct mxc_epdc_fb_data *)info;
	const struct firmware *fw = NULL; /* Lab126 */
	...
	/*
	 * Lab126: Use the passed-in waveform file instead of the default one.
	 */
	if (fb_data->wv_file) {
		wv_file = fb_data->wv_file;
		wv_file_size = fb_data->wv_file_size;
		
		printk("mxc_epdc_fb_init_hw: %s\n", fb_data->wv_file_name);
		goto skip_request_firmware;
	}
	...
	wv_file_size = fw->size; /* Lab126 */

/* Lab126 */
skip_request_firmware:
...
int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
  ...
  /* TODO: Nadim */
	...
	fb_data->auto_mode = AUTO_UPDATE_MODE_AUTOMATIC_MODE_PART;
	...
	fb_data->power_state = POWER_STATE_OFF;
	fb_data->powering_down = false;
	fb_data->pwrdown_delay = 0; /* Lab126 */
...
static int mxc_epdc_fb_remove(struct platform_device *pdev)
{
	...
	/* Lab126 */
	if (fb_data->power_state == POWER_STATE_ON) {
		bool powerdown_disabled = fb_data->pwrdown_delay == FB_POWERDOWN_DISABLE;
		unsigned long start_time = jiffies, stop_time = start_time + HZ;
		
		if (fb_data->pwrdown_delay && !powerdown_disabled) {
			stop_time += msecs_to_jiffies(fb_data->pwrdown_delay);
		}
		
		if (!powerdown_disabled) {
			dev_err(fb_data->dev, "waiting for epdc to power down\n");
			while ((fb_data->power_state == POWER_STATE_ON) && time_before_eq(jiffies, stop_time)) {
				schedule_timeout(HZ/10);
			}
		}
		
		if (powerdown_disabled || time_after(jiffies, stop_time)) {
			dev_err(fb_data->dev, "epdc isn't powered down\n");
			dev_err(fb_data->dev, "clearing interrupts\n");
			epdc_clear_interrupts();
			
			dev_err(fb_data->dev, "forcing power down\n");
			fb_data->powering_down = true;
			epdc_powerdown(fb_data);
		}
	}
...
void mxc_epdc_blit_to_fb(u8 *buffer, struct mxcfb_rect *rect)
{
	if (buffer && rect) {
		struct mxc_epdc_fb_data *fb_data = g_fb_data;
		...
		*/
		for (i = 0, y = ystart; i < yend; i++, y++) {
			memcpy(&fb[(rowbytes_fb * y) + xstart], &buffer[rowbytes_buffer * i], rowbytes_buffer);
		}
	}
}

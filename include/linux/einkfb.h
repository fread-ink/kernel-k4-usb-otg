...
enum fx_type
{
    ...
    fx_update_fast_invert = 4,              // Fast inverted update

    fx_update_white_trans = 5,              // High fidelity from white transition
    ...
    fx_display_sync = 102                   // Wait for any pending display-updates to complete.
};
...
#define UPDATE_MODE_BUFFER_DISPLAY(f)       \
    ((fx_buffer_display_partial == (f)) ||  \
     (fx_buffer_display_full == (f)))

#define UPDATE_MODE_BUFFER(f)               \
    ((fx_buffer_load == (f))            ||  \
     UPDATE_MODE_BUFFER_DISPLAY(f))
...
enum einkfb_events_t
{
    ...
    einkfb_event_update_display_complete,   // The last update_display event is complete.
...
enum display_paused_t
{
    display_resumed,
    display_paused
};
typedef enum display_paused_t display_paused_t;
...
#define EINK_WF_PATH                        "/proc/wf/path"
#define EINK_WF_TYPE                        "/proc/wf/type"
#define EINK_WF_VERSION                     "/proc/wf/version"
#define EINK_WF_EMBEDDED_CHECKSUM           "/proc/wf/embedded_checksum"
#define EINK_WF_COMPUTED_CHECKSUM           "/proc/wf/computed_checksum"

#define EINK_WF_PATH_LEN                    256
#define EINK_WF_TYPE_LEN                    2
#define EINK_WF_VERS_LEN                    64
#define EINK_WF_CKSM_LEN                    16
...
#define FBIO_EINK_SET_PAUSE_RESUME          _IO(FBIO_MAGIC_NUMBER, 0xf6) // 0x46f6 (display_paused_t)
#define FBIO_EINK_GET_PAUSE_RESUME          _IO(FBIO_MAGIC_NUMBER, 0xf7) // 0x46f7 (display_paused_t *)

#ifndef FBIO_WAITFORVSYNC
#define FBIO_WAITFORVSYNC                   _IO(FBIO_MAGIC_NUMBER, 0x20) // 0x4620
#endif
...
#define FBIO_EINK_PROGRESSBAR_BACKGROUND    _IO(FBIO_MAGIC_NUMBER, 0xf4) // 0x46f4 (int: EINK_WHITE || EINK_BLACK)
...
#define PROC_EINK_PAUSE_RESUME_DISPLAY     20   // FBIO_EINK_SET_PAUSE_RESUME
#define PROC_EINK_WAITFORVSYNC             21   // FBIO_WAITFORVSYNC
